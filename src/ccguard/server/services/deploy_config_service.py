...
# 3. Забрать у пользователя право писать в то, что запускает хук.
# Без этого шага защита половинчатая: сам хук лежит под правами
# администратора и подменить его нельзя, но запускает он код из каталога
# агента — а тот при обычной установке принадлежит пользователю. Атакующий
# под той же учётной записью просто переписал бы этот код, и защита
# исполнила бы его сама.
if [ -d {shlex.quote(root)} ]; then
    chown -R root:{group} {shlex.quote(root)}
    chmod -R go-w {shlex.quote(root)}
fi

echo "ccguard: конфигурация раскатана."
echo "Осталось положить исполняемые файлы агента в {root}/bin и запустить службу."
echo "После этого повторите шаг 3, иначе новые файлы останутся доступными на запись."
"""
