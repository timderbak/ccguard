...
## Коммуникация со мной (Тим)

- **Всегда переводи англоязычные и технические термины.** При первом употреблении в ответе давай короткий перевод/расшифровку в скобках. Примеры: anti-tamper (защита от вмешательства/отключения), tamper-resistance (устойчивость к вмешательству — сломать реально трудно), tamper-evidence (фиксация факта вмешательства — сломать можно, но мы мгновенно увидим), same-user (под тем же пользователем), fail-open (отказ в сторону «пропустить»), fail-closed (отказ в сторону «заблокировать»), heartbeat (сигнал «я жив»), watchdog (сторож-процесс). Тим учит термины по ходу — перевод помогает читать и запоминать.
- Расшифровывай аббревиатуры; не пиши простынями; не убегай от поставленного scope (объёма задачи).
